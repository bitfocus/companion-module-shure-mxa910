...
instance.prototype.processShureCommand = function (command) {
	...
	try {
		if (command.substr(0, 3) === 'REP') {
			...
			switch(commandVar) {
				...
				case 'AUDIO_GAIN_HI_RES':
					self.updateVariable('audio_gain_hi_res_' + commandNum, commandVal.trim());
					break;
				case 'AUDIO_MUTE':
					self.updateVariable('channel_mute_' + commandNum, commandVal);
					self.checkFeedbacks('channel_mute');
					break;
				case 'DEVICE_AUDIO_MUTE':
					self.updateVariable('device_audio_mute_', commandVal);
		...
		self.log('error', 'Unexpected error occured processing message from device: ' + error);
	}	
};
...
instance.prototype.init_feedbacks = function() {
	...
	feedbacks['device_audio_mute'] = {
		...
		description: 'If the device is muted, change the color of the button.',
		options: [
			{
				type: 'colorpicker',
				label: 'Foreground color',
	...
	feedbacks['flash_state'] = {
		label: 'Flash State',
		description: 'If the microphone is flashing, change the color of the button.',
		options: [
			{
				type: 'colorpicker',
				label: 'Foreground color',

...
class MXAInstance extends InstanceBase {
			...
			if (command.substring(0, 3) === 'REP') {
				...
				switch(commandVar) {
					...
					case 'IP_ADDR_NET_AUDIO_PRIMARY':
						this.updateVariable('ipaddress_audio_primary', this.trimShureString(commandVal))
						break;
					case 'IP_SUBNET_NET_AUDIO_PRIMARY':
						this.updateVariable('subnet_audio_primary', this.trimShureString(commandVal))
						break;
					case 'IP_GATEWAY_NET_AUDIO_PRIMARY':
						this.updateVariable('gateway_audio_primary', this.trimShureString(commandVal))
						break;	
					case 'DEVICE_ID':
					...
					case 'AUDIO_GAIN_HI_RES':
						self.updateVariable('audio_gain_hi_res_' + channelNumber, this.trimShureString(commandVal))
						break;
					case 'AUDIO_MUTE':
					...
					case 'DEVICE_AUDIO_MUTE':
						this.updateVariable('device_audio_mute', commandVal)
					...
					case 'PRESET_NAME':
						try {
							let [_match, number, name] = commandVal.match(/(\d+) \{(.+?)\s*\}/)
							self.updateVariable('preset_name_' + parseInt(number), name)
							// this.updateActions()
							// this.updateFeedbacks()
						} catch (error) {}
						break;
...
runEntrypoint(MXAInstance, UpgradeScripts)
